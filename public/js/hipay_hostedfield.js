  ...
  configs.forEach((config) => {
    ...
    formPaymentMethod.addEventListener('submit', function(event) {
      ...
      if (document.querySelector('input[name^="sylius_checkout_select_payment[payments]"][type="radio"]') !== null) {
        paymentMethod = document.querySelector('input[name^="sylius_checkout_select_payment[payments]"][type="radio"]:checked')
          .closest('div.item')
          .querySelector('.hipay-div-config')
          .dataset.configGateway;
      }
      ...
      if (paymentMethod === gateway) {
        ...
        cardHipay.getPaymentData().then(
          ...
          function(error) {
            alert('An error occurred while trying to obtain Hipay response.');
            console.error(error);
          },
        );
      }
